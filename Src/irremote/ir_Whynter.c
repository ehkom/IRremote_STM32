...
void  sendWhynter (unsigned long data,  int nbits)
{
	...
	// Start
    irsend_mark(WHYNTER_ZERO_MARK);
    irsend_space(WHYNTER_ZERO_SPACE);

	// Header
    irsend_mark(WHYNTER_HDR_MARK);
    irsend_space(WHYNTER_HDR_SPACE);

	// Data
    unsigned long  mask;
    for (mask = 1UL << (nbits - 1);  mask;  mask >>= 1) {
		if (data & mask) {
            irsend_mark(WHYNTER_ONE_MARK);
            irsend_space(WHYNTER_ONE_SPACE);
		} else {
            irsend_mark(WHYNTER_ZERO_MARK);
            irsend_space(WHYNTER_ZERO_SPACE);
		}
	...
	// Footer
    irsend_mark(WHYNTER_ZERO_MARK);
    irsend_space(WHYNTER_ZERO_SPACE);  // Always end with the LED off
}
#endif
